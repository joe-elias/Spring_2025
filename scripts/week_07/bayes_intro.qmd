...
quantile(post$n_socks, c(.01, .96))
```

What is the probability of having less than 5 odd socks?
...
samp_2 <- pick_11_socks(n_socks = n_socks, prop_pairs = prop_pair)
n_picked <- 11 # The number of socks to pick out of the laundry

post_n_socks <- post$n_socks
post_p_pair <- post$prop_pairs

sock_sim <- replicate(1e5, {
  # Generating a sample of the parameters from the priors
  n_socks <- sample(post_n_socks, 1)
  prop_pairs <- sample(post_p_pair, 1)
  pick_11_socks(n_socks, prop_pairs)
})
sock_sim_2 <- t(sock_sim)

head(sock_sim_2)

post_sock_2 <- sock_sim_2[sock_sim_2[,"unique"]==9 & sock_sim_2[,"pairs"]==1,]

hist(post_sock_2[,"n_socks"])
```
