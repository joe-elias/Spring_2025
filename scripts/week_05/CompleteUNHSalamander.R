...
Sal %>% 
  ggplot(aes(redness, maxsprint, fill=site, col=site, shape = site)) +
  geom_point() +
  theme_light() +
  xlab("Relative Redness") +
  ylab("Maximum Sprint Speed (cm/s)") +
  geom_smooth(method= lm, alpha= 0.2) +
  scale_color_manual(values = c("#648FFF","#DC267F","#785EF0")) +
  scale_fill_manual(values = c("#648FFF","#DC267F","#785EF0"))
...
Sal %>% 
  ggplot(aes(site, redness, fill= site)) +
  xlab("Site") +
  ylab("Relative Redness") +
  geom_violin() +
  theme_light() +
